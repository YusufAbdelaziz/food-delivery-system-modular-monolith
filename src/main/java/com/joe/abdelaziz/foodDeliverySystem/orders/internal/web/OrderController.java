package com.joe.abdelaziz.foodDeliverySystem.orders.internal.web;
...
public class OrderController {
  ...
  public ResponseEntity<OrderDTO> getById(
      @PathVariable @Parameter(description = "Order ID", required = true) Long id) {
  ...
  public ResponseEntity<OrderDTO> changeOrderStatus(
    ...
    if (!node.has("orderId") || !node.has("status")) {
      throw new BusinessLogicException(
          "To change a status of an order to a courier, you have to provide the status and the"
              + " order id");
    }
    try {
      ...
      throw new BusinessLogicException(
          "Unable to parse the status, make sure to provide a valid value please", e);
    }
  }
