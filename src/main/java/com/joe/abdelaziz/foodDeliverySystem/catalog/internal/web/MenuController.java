...
import com.joe.abdelaziz.foodDeliverySystem.catalog.api.service.MenuService;
...
public class MenuController {
  ...
  public ResponseEntity<MenuDTO> createMenu(
      @RequestBody @Parameter(description = "Menu data to be created", required = true) @Valid
          MenuDTO menu) {
    return ResponseEntity.status(HttpStatus.CREATED).body(menuService.insertMenu(menu));
  }

  @Operation(
      summary = "Retrieve a menu by restaurant ID",
      description = "Fetches the menu associated with a specific restaurant ID")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            ...
            content =
                @Content(array = @ArraySchema(schema = @Schema(implementation = MenuDTO.class)))),
      })
  @PreAuthorize("hasRole('ADMIN')")
  @GetMapping("")
  ...
  @Operation(
      ...
      description = "Removes a menu from the system by its ID")
  @ApiResponses(
      value = {
