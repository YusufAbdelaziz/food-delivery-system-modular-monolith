...
@Mapper(
    uses = {SpecOptionMapper.class},
    nullValuePropertyMappingStrategy = NullValuePropertyMappingStrategy.IGNORE)
public abstract class SpecMapper {

  @Autowired private SpecOptionMapper specOptionMapper;
  ...
  @Mappings(
      value = {
        ...
        @Mapping(
            target = "options",
            expression = "java(updateSpecOptions(dto.getOptions(), existingSpec))")
      })
  ...
  public Set<SpecOption> updateSpecOptions(Set<SpecOptionDTO> optionsDTOs, Spec existingSpec) {
    ...
    Map<Long, SpecOption> existingSpecOptionMap =
        existingSpecOptions.stream()
    ...
    for (SpecOptionDTO specOptionDTO : optionsDTOs) {
      ...
      if (specOptionDTO.getId() != null
          && existingSpecOptionMap.containsKey(specOptionDTO.getId())) {
