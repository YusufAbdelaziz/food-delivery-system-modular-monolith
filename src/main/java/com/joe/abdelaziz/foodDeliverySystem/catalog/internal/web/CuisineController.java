...
import com.joe.abdelaziz.foodDeliverySystem.catalog.api.service.CuisineService;
...
public class CuisineController {
  ...
  public ResponseEntity<CuisineDTO> addCuisine(
      @Valid @RequestBody @Parameter(description = "Cuisine data", required = true)
          CuisineDTO cuisine) {
  ...
  @Operation(
      ...
      description = "Fetches details of a specific cuisine by its ID")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            ...
            content =
                @Content(
                    array = @ArraySchema(schema = @Schema(implementation = CuisineDTO.class)))),
        @ApiResponse(responseCode = "403", description = "User is not authorized")
      })
  @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
  @GetMapping("")
