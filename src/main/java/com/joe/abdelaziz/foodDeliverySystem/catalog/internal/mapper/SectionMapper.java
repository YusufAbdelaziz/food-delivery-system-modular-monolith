...
public abstract class SectionMapper {
  ...
  @Mappings(
      value = {
        ...
        @Mapping(
            target = "items",
            expression = "java(updateItems(dto.getItems(), existingSection))")
      })
  ...
  public Set<Item> updateItems(Set<ItemDTO> itemDTOs, Section existingSection) {
    ...
    Map<Long, Item> existingItemMap =
        existingItems.stream().collect(Collectors.toMap(Item::getId, Function.identity()));
