package com.joe.abdelaziz.foodDeliverySystem.customer.internal.web;
...
public class CustomerController {
  ...
  public ResponseEntity<CustomerDTO> createCustomer(
      @RequestBody @Valid @Parameter(description = "Customer details", required = true) CustomerDTO user) {
    log.info("Request received: " + user);
    return ResponseEntity.ok().body(customerService.insertCustomer(user));
  }
  ...
  public ResponseEntity<CustomerDTO> getCustomerById(
    ...
    return ResponseEntity.ok().body(customerService.findDtoById(id));
  }

  @Operation(summary = "Get all customers", description = "Retrieves a list of all customers")
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "List of customers retrieved successfully", content = @Content(schema = @Schema(implementation = CustomerDTO.class), array = @ArraySchema(schema = @Schema(implementation = CustomerDTO.class)))),
  ...
  @Operation(summary = "Delete customer address", description = "Deletes an address owned by the specified customer")
  @ApiResponses(value = {
  ...
  public ResponseEntity<Void> deleteOwnAddress(
      @PathVariable @Parameter(description = "Customer ID", required = true) Long customerId,
      @PathVariable @Parameter(description = "Address ID", required = true) Long addressId) {
    customerService.removeOwnAddress(customerId, addressId);
